...
(function($) {
    var methods = {
        init: function(useroptions) {
            ...
            var overlayTemplate = '<div class="loadgo-overlay" style="background-color:%bgcolor%;opacity:%opacity%;width:%width%px;height:%height%px;position:absolute;z-index:4"></div>';
            var overlayWithOptions = overlayTemplate.replace('%bgcolor%', '#000').replace('%opacity%', '0.5').replace('%width%', _w).replace('%height%', _h);
            ...
            if (pluginOptions.filter === null) {
                $this.wrapAll('<div class="loadgo-container" style="position: absolute"></div>');
        ...
        destroy: function() {
            ...
            if (options.overlay) {
                ...
                $this.siblings('.loadgo-container').remove()
            }
